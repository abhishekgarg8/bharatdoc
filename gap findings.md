...
- Remaining findings are reprioritized by production risk: P1, P2, then P3.
- No active P0 findings are known.

## P1 - Production Risk / Data Integrity / Security

### P1-1. Onboarding writes are still not transactional
...
### P1-2. Generic server errors are returned to clients verbatim

Files:
- `apps/web/lib/server/errors.ts`
- `apps/worker/src/http-errors.ts`

Problem:
- Unknown `Error` objects are converted to API responses with `error.message`.
- Supabase, storage, OpenAI, and other internal errors can leak implementation details to the browser.
- Worker telemetry sanitizes internal errors, but the HTTP response still returns the raw message.
...
### P1-3. Expired/invalid sessions leave users on dead-end error screens
...
- Other protected page clients using `PageError`

Problem:
- When a stale Supabase session token causes a 401/API failure, protected pages render `PageError`.
- `PageError` has no recovery action.
- Users see messages like "Please sign in again" but have no sign-in button or automatic redirect.

Fix:
- Detect 401 responses in shared API parsing or page-client catch blocks.
- Call `client.signOut()` and redirect to `/onboarding` for expired sessions.
- Add a "Sign in again" action to `PageError` as a fallback.
- Add tests for expired-token behavior on dashboard, settings, pending approval, search, and recording detail.

### P1-4. Public clinic-code lookup has no abuse guard

Files:
- `apps/web/app/api/clinics/lookup/route.ts`
